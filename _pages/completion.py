...
sections = [0, 0, 0, 0]  # for 4 levels of hierarchy

def replacer(match):
    level = len(match.group(1))
    sections[level-1] += 1
    for i in range(level, len(sections)):
        sections[i] = 0
    return '.'.join(str(s) for s in sections if s != 0) + ' ' + match.group(2)
...
for idx, item in enumerate(items):
    new_text = new_text.replace('- ' + item, f'1.{idx + 1} {item}', 1)
